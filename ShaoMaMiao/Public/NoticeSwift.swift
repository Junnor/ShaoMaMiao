...
class SwiftNotice: NSObject {
    ...
    static func noticeOnStatusBar(_ text: String, autoClear: Bool, autoClearTime: Int) {
        ...
        if let version = Double(UIDevice.current.systemVersion),
            ...
            // change direction
            let mpi = CGFloat.pi
            window.transform = CGAffineTransform(rotationAngle: CGFloat(degree) * mpi / 180)
        }
    ...
    static func wait(_ imageNames: Array<UIImage> = Array<UIImage>(), timeInterval: Int = 0) {
        ...
        if let version = Double(UIDevice.current.systemVersion),
            ...
            // change direction
            window.transform = CGAffineTransform(rotationAngle: CGFloat(degree) * CGFloat.pi / 180)
        }
    ...
    static func showNoticeWithText(_ type: NoticeType,text: String, autoClear: Bool, autoClearTime: Int) {
        let frame = CGRect(x: 0, y: 0, width: 90, height: 106)
        let window = UIWindow()
        window.backgroundColor = UIColor.clear
        let mainView = UIView()
        ...
        switch type {
        case .success:
            mainView.backgroundColor = UIColor(red:47/255.0, green:211/255.0, blue:29/255.0, alpha: 0.85)
        default:
            mainView.backgroundColor = UIColor(red:0/255.0, green:0/255.0, blue:7/255.0, alpha: 0.85)
        }
        ...
        let label = UILabel(frame: CGRect(x: 0, y: 60, width: 90, height: 32))
        label.font = UIFont.systemFont(ofSize: 13)
        label.textColor = UIColor.white
        label.text = text
        ...
        //这儿增加一个label自适应长度，有需求就改掉
        label.adjustsFontSizeToFitWidth = true
        //        checkmarkView.frame = CGRect(x: (label.frame.size.width - 36)/2, y: 15, width: 36, height: 36)
        //        frame.size = CGSize.init(width: label.frame.size.width, height: frame.size.height)
...
class SwiftNoticeSDK {
    ...
    class func draw(_ type: NoticeType) {
        ...
        checkmarkShapePath.addArc(withCenter: CGPoint(x: 18, y: 18), radius: 17.5, startAngle: 0, endAngle: CGFloat.pi*2, clockwise: true)
        ...
        case .info:
            ...
            checkmarkShapePath.addArc(withCenter: CGPoint(x: 18, y: 27), radius: 1, startAngle: 0, endAngle: CGFloat.pi*2, clockwise: true)
            checkmarkShapePath.close()
